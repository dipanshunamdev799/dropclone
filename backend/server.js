...
    console.error('Login error:', error);
    // FIXED: Pass the specific error message from Cognito to the frontend.
    // This allows the frontend to handle cases like "User is not confirmed."
    res.status(401).json({ error: error.message || 'Invalid credentials' });
  }
});

// Verify user email (if auto-verification is disabled)
